...
// connection that hadn't stabilized yet.
//
// FIX: raised from 3 attempts / linear 300ms backoff to 6 attempts /
// exponential backoff (capped at 4s). On a free-tier host (Render/Railway
// free plans spin down after ~15 min idle), the FIRST request after the
// app comes back from the background often has to both wake the server
// process AND do a fresh MongoDB Atlas SRV lookup + TLS handshake — that
// routinely takes longer than the old ~1.2s total retry budget, which is
// exactly why "Stop not saved: ConnectionException: connection closed"
// was showing up specifically after backgrounding the app.
Future<T> _withRetry<T>(Future<T> Function() action, {int maxAttempts = 6}) async {
  var attempt = 0;
  while (true) {
    try {
      return await action();
      ...
      print("Query failed ($e). Reconnecting to MongoDB and retrying (attempt $attempt)...");
      await _reconnect();
      final delayMs = (500 * math.pow(2, attempt - 1)).clamp(500, 4000).toInt();
...
// 3. MONGODB CHANGE STREAM -> MQTT BRIDGE WORKER
// ==========================================
// FIX: this used to be a single one-shot subscription — the first time
// Atlas reset the underlying socket (which it does periodically; this is
// normal, not a bug), the stream died silently (or, worse, mongo_dart's
// own internal reconnect attempt threw asynchronously with nothing
// listening for it, which is what crashed the whole process — see the
// runZonedGuarded wrapper around main() below, which is the other half of
// this fix). Now every failure — whether reported through onError or via
// the stream just ending (onDone) — triggers a fresh watch() after a
// short delay instead of leaving the bridge permanently dead.
Future<void> startMongoChangeStreamBridge() async {
  await _watchChangeStreamWithRestart();
}

Future<void> _watchChangeStreamWithRestart() async {
  StreamSubscription? sub;

  void restart(String reason) {
    print("[MQTT BRIDGE] Change stream ended ($reason). Restarting in 3s...");
    sub?.cancel();
    Future.delayed(const Duration(seconds: 3), _watchChangeStreamWithRestart);
  }
  ...
  try {
    ...
    sub = stream.listen(
      ...
      onDone: () => restart("stream closed"),
      cancelOnError: true,
    );
  } catch (e) {
    print("[MQTT BRIDGE ERROR] Failed to open change stream: $e");
    Future.delayed(const Duration(seconds: 3), _watchChangeStreamWithRestart);
  }
}
...
// 5. MAIN SERVICE DRIVER Entrypoint
// ==========================================
// FIX: previously `main()` ran everything directly, so an unhandled async
// error ANYWHERE in the process — including inside mongo_dart's own
// internal socket-reconnect logic, which runs on its own outside any of
// our try/catch blocks — would crash the entire Dart process. That's
// exactly what the "Unhandled exception: MongoDB ConnectionException:
// Socket error... Connection reset by peer" crash in the logs was: Atlas
// reset an idle socket (normal), the driver's own reconnect attempt threw
// asynchronously, nothing was listening for it, and the whole server went
// down and had to be restarted by Render — taking every in-flight request
// (login, add-machine-record, everything) down with it for several
// seconds.
//
// runZonedGuarded catches any error that would otherwise be unhandled
// anywhere in this zone (including ones not tied to a specific
// try/catch), logs it, and lets the process keep running instead of
// dying. This is a safety net on top of (not a replacement for) the
// change-stream self-restart above and the query retry logic elsewhere —
// it's what stops one bad socket event from taking the whole server
// offline.
void main() {
  runZonedGuarded(() async {
    await _startServer();
  }, (error, stackTrace) {
    print("[FATAL - CAUGHT BY ZONE GUARD] Unhandled error: $error");
    print(stackTrace);
    print("[FATAL - CAUGHT BY ZONE GUARD] Server process is still alive; continuing.");
  });
}
